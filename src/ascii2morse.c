...
//    printf("prog1 says \"%s\"\n", message);
    ...
    for (int i = 0; i < nread - 1; i++) {
        char upper_character = (char)toupper(chars[i]);
        node.data = upper_character;
        node.morse = getMorseFromHashTable(node.data);
        addElement(pList, i, node);
    }

    displayLinkedListAlphabet(pList);
    displayLinkedListMorse(pList);
    morseProcessing(pList);
